...
class Face_Recognizer:
    ...
    def get_face_database(self):
        if os.path.exists("data/features_all.csv"):
            ...
            logging.info("Recognition threshold set to 0.5")
            return 1
        else:
                ...
                for k in range(len(faces)):
                    ...
                    for i in range(len(self.face_features_known_list)):
                        if str(self.face_features_known_list[i][0]) != '0.0':
                            e_distances.append(
                                self.return_euclidean_distance(self.current_frame_face_feature_list[k], self.face_features_known_list[i])
                            )
                        else:
                    ...
                    if min_dist < 0.5:
                        best_match = e_distances.index(min_dist)
                        self.current_frame_face_name_list[k] = self.face_name_known_list[best_match]
                        self.attendance(self.face_name_known_list[best_match])
                    else:
